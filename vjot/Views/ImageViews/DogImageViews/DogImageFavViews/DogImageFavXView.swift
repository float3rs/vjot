...
struct DogImageFavXView: View {
    var body: some View {
        DogImageFavXListView()
    }
}
...
struct DogImageFavXListView: View {
                            ...
                            Section(header: Text(veterinarianGig.track(currentId: uuid)?.name ?? uuid.uuidString)) {
                                ...
                                if let favs = dict[uuid] {
                                    ...
                                    ForEach(favs, id: \.id) { fav in
                                        ...
                                        NavigationLink {
                                            DogImageFavXListElementContentView(
                                                favourite: fav,
                                                veterinarianId: uuid
                                            )
                                        } label: {
                                            DogImageFavXListElementView(favourite: fav)
                                        }
                                    }
                                }
                            }
...
struct DogImageFavXListElementView: View {
...
struct DogImageFavXListElementContentView: View {
    var favourite: DogAPIFavourite
                    ...
                    Group {
                        if veterinarianGig.track(currentId: veterinarianId) == nil {
                            Button {
                                Task {
                                    do {
                                        let _ = try await favouriteEngine.deleteDog(
...
//struct DogImageFav2View_Previews: PreviewProvider {
