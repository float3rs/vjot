...
struct DogImageSearchView: View {
        ...
        VStack {
            ...
            ScrollView {
                VStack(alignment: .center, spacing: 0) {
                    ...
                    VStack(spacing: -10) {
                        Text("search dog images")
                        if (
                            !imageEngine.dogBreedIds.isEmpty ||
                            !imageEngine.dogCategoryIds.isEmpty ||
                            typesShown ||
                            sizesShown ||
                            mixedExcluded
                        ) {
                    ...
                    Button {
                        Task {
                            do {
                                ...
                                images = try await imageEngine.fetchDogs(
                                    typesShown: typesShown,
                                    type: type,
                                    size: size,
                                    count: count,
...
struct DogImageSearchOptionsView: View {
        ...
        ScrollView {
            ...
            if !breedEngine.dogBreeds.isEmpty {
                VStack {
                    ...
                    if !imageEngine.dogBreedIds.isEmpty {
                        VStack(alignment: .vjotImageH) {
                            ForEach(Array(imageEngine.dogBreedIds.enumerated()), id: \.offset) { index, breedId in
                    ...
                    if categoriesShown {
                        VStack(alignment: .vjotImageH) {
                            ForEach(categoryEngine.dogCategories) { category in
                                HStack {
                                    Text(category.name)
                                    if imageEngine.dogCategoryIds.contains(where: { $0 == category.id }) {
                                        ...
                                        Button {
                                            imageEngine.dogCategoryIds = imageEngine.dogCategoryIds.filter { $0 != category.id }
                                        } label: {
                                        ...
                                        Button {
                                            imageEngine.dogCategoryIds.append(category.id)
                                        } label: {
...
//struct DogImageSearchView_Previews: PreviewProvider {
