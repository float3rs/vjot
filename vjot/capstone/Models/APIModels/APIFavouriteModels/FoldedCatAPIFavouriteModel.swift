//
//  FoldedCatAPIFavouriteModel.swift
//  Saridakis-vjot
//
...
// MARK: CAT'S FAVOURITES (DECODING)

struct FoldedCatAPIFavourite {
    var id: Int
    var userId: String?         // NOT PRESENT IN DOCUMENTATION
    var imageId: String
    var subId: String
    var createdAt: Date
    var image: FoldedCatAPIImage?
}

extension FoldedCatAPIFavourite: Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        do {
            ...
            report(error: error, decoding: "FoldedCatAPIFavourite.id")
            throw(error)
        }
        do {
            userId = try container.decodeIfPresent(String.self, forKey: .userId) ?? nil
        } catch(let error) {
            report(error: error, decoding: "FoldedCatAPIFavourite.userId")
            throw(error)
        }
        do {
...
func decodeCatAPIFavourites(data: Data) throws -> [FoldedCatAPIFavourite] {
    let jsonData = data
    ...
    let foldedCatAPIFavourites = try decoder.decode([FoldedCatAPIFavourite].self, from: jsonData)
    return foldedCatAPIFavourites
}

// ----------------------------------------------------------------------------

func decodeCatAPIIndividualFavourite(data: Data) throws -> FoldedCatAPIFavourite {
    let jsonData = data
    let decoder = JSONDecoder()
    ...
    let foldedCatAPIIndividualFavourite = try decoder.decode(FoldedCatAPIFavourite.self, from: jsonData)
    return foldedCatAPIIndividualFavourite
}
