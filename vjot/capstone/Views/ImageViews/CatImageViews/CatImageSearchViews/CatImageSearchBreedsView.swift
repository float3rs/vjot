//
//  CatImageSearchBreedsView.swift
//  Saridakis-vjot
//
...
struct CatImageSearchBreedsView: View {
...
struct CatImageSearchBreedsListView: View {
                ...
                if searchState == 0 {
                    ForEach(breedEngine.searchCats()) { catBreed in
                        NavigationLink {
                            ...
                            CatImageSearchBreedsListElementView(catBreed: catBreed)
                        }
                        ...
                        .swipeActions(edge: .leading, allowsFullSwipe: true, content: {
                            Button {
                                imageEngine.catBreedIds.append(catBreed.id)
                            } label: {
                        ...
                        .swipeActions(edge: .trailing, allowsFullSwipe: true, content: {
                            Button {
                                imageEngine.catBreedIds = imageEngine.catBreedIds.filter { $0 != catBreed.id }
                            } label: {
...
struct CatImageSearchBreedsListElementView: View {
    ...
    var body: some View {
        HStack {
            Spacer()
            Text(catBreed.name)
                .foregroundColor(imageEngine.catBreedIds.contains(where: {$0 == catBreed.id}) ? Color.accentColor : Color.secondary)
        }
    }
}
...
//struct CatImageBreedsView_Previews: PreviewProvider {
