...
struct DogVoteButtonView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $goVote, content: {
            DogVoteSheetView(image: image, voted: $voted, goVote: $goVote)
        })
        ...
        .onChange(of: image, perform: { newValue in
            if let currentId = veterinarianGig.currentId {
                if let currentVet = veterinarianGig.track(currentId:currentId) {
                    if let voteId = currentVet.dogVotes[newValue.id] {
                        voted = true
                        self.voteId = voteId
                    } else {
                        voted = false
...
//struct DogVoteButtonView_Previews: PreviewProvider {
