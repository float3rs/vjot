...
struct CatFavButtonView: View {
    ...
    var body: some View {
                        ...
                        Button {
                            Task {
                                do {
                                    ...
                                    if favouriteId != nil {
                                        deleteFavResponse = try await favouriteEngine.deleteCat(
                                        ...
                                        if deleteFavResponse != nil {
                                            ...
                                            currentVet.catFavs.removeValue(forKey: image.id)
                        ...
                        Button {
                            Task {
                                do {
                                    ...
                                    postFavResponse = try await favouriteEngine.postCat(
                                        imageId: image.id,
                                        subId: currentId
                                    )
                                    ...
                                    if let postFavResponse {
                                        ...
                                        currentVet.catFavs[image.id] = postFavResponse.id
                                        try veterinarianGig.update(veterinarian: currentVet)
        ...
        .onAppear {
            if let currentId = veterinarianGig.currentId {
                if let currentVet = veterinarianGig.track(currentId:currentId) {
                    if let favId = currentVet.catFavs[image.id] {
                        faved = true
                        favouriteId = favId
                    } else {
                        faved = false
        ...
        .onChange(of: image, perform: { newValue in
            if let currentId = veterinarianGig.currentId {
                if let currentVet = veterinarianGig.track(currentId:currentId) {
                    if let favId = currentVet.catFavs[newValue.id] {
                        faved = true
                        favouriteId = favId
                    } else {
                        faved = false
...
//struct CatFavView_Previews: PreviewProvider {
