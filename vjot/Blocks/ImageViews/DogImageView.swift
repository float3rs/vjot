...
struct DogImageView: View {
    ...
    var body: some View {
            ...
            if let breeds = image.breeds {
                if !breeds.isEmpty {
                    HStack {
                        ...
                        if let breedName = breedEngine.dogCorrelate()[breeds[0].id] {
                            Text(breedName)
                        }
                    }
                    .font(.subheadline)
            ...
            if let categories = image.categories {
                if !categories.isEmpty {
                    HStack {
                        Text("category:")
                            .foregroundColor(Color.secondary)
                        if let categoryName = categoryEngine.dogCorrelate()[categories[0].id] {
                            Text(categoryName)
                        } else {
        ...
        .sheet(isPresented: $goAnalyze) {
            DogAnalysisSheetView(image: image, goAnalyze: $goAnalyze)
        }
